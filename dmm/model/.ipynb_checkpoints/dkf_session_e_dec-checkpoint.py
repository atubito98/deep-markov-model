...
def build_DKF_session_e_dec(cfg, device='cpu'):
    ...
    s_dim = cfg.getint('Network','s_dim')
    num_sessions = cfg.getint('Network','num_sessions')
    ...
    # Build model
    model = DKF_session_e_dec(c_dim=c_dim, x_dim=x_dim, z_dim=z_dim, tau=tau, s_dim=s_dim,
                dense_x_gx=dense_x_gx, dim_RNN_gx=dim_RNN_gx, num_sessions=num_sessions,
...
class DKF_session_e_dec(nn.Module):

    def __init__(self, c_dim, x_dim, z_dim=2, h_gen=16, dense_h=[], tau=4, s_dim=8,
                 dense_x_gx=[], dim_RNN_gx=128, num_RNN_gx=1, dense_ztm1_g=[],
                 num_sessions=12, dropout_p = 0, beta=1, device='cpu'):
        ...
        self.z_dim = z_dim
        self.s_dim = s_dim
        self.num_sessions = num_sessions  # passalo dal costruttore
        self.tau = tau
...
#         self.posterior_mlp = nn.Linear(self.z_dim, self.dense_ztm1_g)
        self.posterior_mlp = nn.Sequential(nn.Linear(self.z_dim, self.dense_ztm1_g),
                                           nn.ReLU(),
...
#                                           )
        ...
        self.FiLM_prior = nn.Sequential(nn.Linear(self.c_dim, self.dense_h * 2),
                                       nn.ReLU(),
                                       nn.Dropout(p=self.dropout_p),
...
#                                             )
        ...
        self.FiLM_gen = nn.Sequential(nn.Linear(self.s_dim, self.dense_h*2),
                                       nn.ReLU(),
                                       nn.Dropout(p=self.dropout_p),
                                       #nn.Linear(64, self.dense_h * 2),
                                       #nn.ReLU(),
...
#         )
    ...
    def generation_z(self, z_tm1, c):          # z_tm1 sarebbe z_{t-1}
        ...
        gamma, beta = self.FiLM_prior(c).chunk(2, dim=-1)
    ...
    def generation_x(self, z, s_expand):
        
        # 1. z_t to y_t
        gamma, beta = self.FiLM_gen(s_expand).chunk(2, dim=-1)
        h_t = self.mlp_z_x(z) 
        y = gamma * h_t + beta 
    ...
    def forward(self, x, c, session_ids=None, s_override=None):
        
        # need input:  (seq_len, batch_size, x_dim)
        seq_len, batch_size, _ = x.shape
        
        # --- Choose the source of session information ---
        if s_override is not None:
            # manually provided embedding (used for unseen sessions)
            if s_override.dim() == 2 and s_override.size(0) == 1:
                s = s_override.expand(batch_size, -1)
            else:
                s = s_override
        else:
            # use learned embeddings from training
            s = self.session_embeddings(session_ids) # (batch, s_dim)
        
        s_expand = s.unsqueeze(0).expand(seq_len, -1, -1)
        ...
        gen_mean, gen_logvar = self.generation_x(self.z, s_expand)
