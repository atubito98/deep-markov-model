...
def build_DKF_session_hu(cfg, device='cpu'):
    ...
    # Build model
    model = DKF_session_hu(c_dim=c_dim, x_dim=x_dim, z_dim=z_dim, tau=tau,
...
class DKF_session_hu(nn.Module):
    ...
    def __init__(self, c_dim, x_dim, z_dim=2, s_dim=8, h_gen=16, dense_h=[128], tau=4,
                 ...
                 sess_encoder_layers=1, sess_encoder_pool='mean'):
...
#         )
    ...
    def generation_z(self, z_tm1, s, c_t):          # z_tm1 sarebbe z_{t-1}
    ...
    def generation_x(self, z, s_expand, c):
...
#             kl_s = torch.tensor(0., device=device)
        ...
        self.z_mean_p, self.z_cov_p = self.generation_z(z_tm1, s_expand, c)  
        gen_mean, gen_logvar = self.generation_x(self.z, s_expand, c)
