...
def build_DKF_session_all(cfg, device='cpu'):
    ...
    # Build model
    model = DKF_session_all(c_dim=c_dim, x_dim=x_dim, z_dim=z_dim, tau=tau,
...
class DKF_session_all(nn.Module):
    ...
    def build(self):
        ...
        self.rnn_gx = nn.LSTM((self.dense_x_gx + self.c_dim), self.dim_RNN_gx, self.num_RNN_gx)
        ...
        self.posterior_gru = nn.GRUCell(self.dim_RNN_gx, self.dense_h)
    ...
    def inference(self, x, c, session_id):
        ...
        h_input = F.relu(self.input_adapters[sid](self.input_dropout(x)))
...
#             h_z = self.hidden_layer(g_z)
    ...
    def forward(self, x, c, session_id):
        ...
        self.z, self.z_mean, self.z_cov = self.inference(x, c, session_id)
