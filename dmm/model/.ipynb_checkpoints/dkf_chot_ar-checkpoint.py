...
def build_DKF_chot_ar(cfg, device='cpu'):
...
#     gamma = cfg.getfloat('Training', 'gamma')
    ...
    # Build model
    model = DKF_chot_ar(c_dim=c_dim, x_dim=x_dim, z_dim=z_dim, tau=tau,
...
#                                           )
        ...
        self.cond_proj = nn.Sequential(nn.Linear(self.c_dim, 64),
                                       nn.ReLU(),
                                       nn.Dropout(p=self.dropout_p),
...
#         )
        ...
        self.mlp_x_prev = nn.Sequential(
            nn.Linear(self.y_dim, self.y_dim),
            nn.Tanh()
        )
    ...
    def generation_x(self, z, x_prev=None, p=0):
        """
        Decoding step: optionally autoregressive on previous x_{t-1}.
        z: (seq_len, batch, z_dim)
        x_prev: (seq_len, batch, x_dim)  -> true previous x (teacher forcing)
                 if None: model runs autoregressively (sampling mode)
        """
        seq_len, batch_size, _ = z.shape
        gen_mean = torch.zeros(seq_len, batch_size, self.y_dim, device=self.device)
        gen_logvar = torch.zeros(seq_len, batch_size, self.y_dim, device=self.device)

        # initialize previous x (zero or first input)
        prev_x = torch.zeros(batch_size, self.y_dim, device=self.device)

        for t in range(seq_len):
            # base mean from z_t
            h = self.mlp_z_x(z[t])
            base_mean = self.gen_mean(h)
            base_logvar = self.gen_logvar(h)

            # AR(1) correction: mu_t += A * x_{t-1}
            mu_t = base_mean + self.mlp_x_prev(prev_x)
            gen_mean[t] = mu_t
            gen_logvar[t] = base_logvar

            # update prev_x (teacher forcing if x_prev available)
            # Scheduled sampling o “noisy teacher forcing”
            if x_prev is not None and t < x_prev.shape[0]:
                use_generated = np.random.rand() < p
                if not use_generated:
                    prev_x = x_prev[t] 
                else:
                    # autoregressive sampling
                    x_t = self.reparameterization(mu_t, base_logvar)
                    prev_x = x_t.detach()
            else:
                # autoregressive sampling
                x_t = self.reparameterization(mu_t, base_logvar)
    ...
    def forward(self, x, c, p=0):
        ...
        gen_mean, gen_logvar = self.generation_x(self.z, x_prev=x, p=p)  # teacher forcing
