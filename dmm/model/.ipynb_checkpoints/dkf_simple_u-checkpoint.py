...
def build_DKF_simple_u(cfg, device='cpu'):
...
#     gamma = cfg.getfloat('Training', 'gamma')
    ...
    # Build model
    model = DKF_simple_u(c_dim=c_dim, x_dim=x_dim, z_dim=z_dim, tau=tau, u_dim=u_dim,
...
class DKF_simple_u(nn.Module):

    def __init__(self, c_dim, x_dim, z_dim=2, h_gen=16, dense_h=[], tau=4, u_dim=3,
...
#         self.dense_ztm1_g = dense_ztm1_g[-1]
    ...
    def build(self):
        ...
        self.posterior_gru_u = nn.GRUCell(self.dense_h + self.dense_h, self.dense_h)
...
#             h_z = self.hidden_layer(g_z)
    ...
    def forward(self, x, c):
        ...
        self.z, self.z_mean, self.z_cov, self.u, self.u_mean, self.u_logvar = self.inference(x, c)
