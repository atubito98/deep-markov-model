...
def build_DKF_chot_new(cfg, device='cpu'):
...
#     gamma = cfg.getfloat('Training', 'gamma')
    ...
    # Build model
    model = DKF_chot_new(c_dim=c_dim, x_dim=x_dim, z_dim=z_dim, tau=tau,
...
class DKF_chot_new(nn.Module):
    ...
    def build(self):
        ...
        ###################
        # 1. x and c to rnn
        self.input_embed = nn.Sequential(nn.Linear(self.x_dim + self.c_dim, self.dense_h),
                                         nn.ReLU(),
