...
def build_DKF_session_hs(cfg, device='cpu'):
    ...
    # Build model
    model = DKF_session_hs(c_dim=c_dim, x_dim=x_dim, z_dim=z_dim, tau=tau,
                dense_x_gx=dense_x_gx, dim_RNN_gx=dim_RNN_gx, 
                num_RNN_gx=num_RNN_gx, dense_h=dense_h, dense_ztm1_g=dense_ztm1_g,
                dropout_p=dropout_p, beta=beta, gamma=gamma, device=device).to(device)

    return model


class DKF_session_hs(nn.Module):

    def __init__(self, c_dim, x_dim, z_dim=2, s_dim=8, h_gen=16, dense_h=[128], tau=4,
                 dense_x_gx=[256], dim_RNN_gx=128, num_RNN_gx=1, dense_ztm1_g=[256],
                 dropout_p = 0, beta=1, device='cpu', sess_encoder_hidden=256, 
                 num_sessions=12, sess_encoder_layers=1, sess_encoder_pool='mean'):
        ...
        # Session encoder params
        self.sess_encoder_hidden = sess_encoder_hidden
        self.sess_encoder_layers = sess_encoder_layers
    ...
    def build(self):
        ...
        self.session_embeddings = nn.Embedding(self.num_sessions, self.s_dim)
        
        # Session encoder (stronger): LSTM aggregator over sample trials
        self.sess_step_embed = nn.Sequential(
            nn.Linear(self.x_dim + self.c_dim, 128),
            nn.ReLU(),
            nn.Dropout(p=self.dropout_p)
        )
        # LSTM aggregator to produce trial-level embedding
        self.sess_lstm = nn.LSTM(input_size=128, hidden_size=self.sess_encoder_hidden,
                                 num_layers=self.sess_encoder_layers, batch_first=False,
                                 bidirectional=False)
        # after pooling trial embeddings, MLP to mean/logvar of s
        self.sess_pool_mlp = nn.Sequential(
            nn.Linear(self.sess_encoder_hidden, self.sess_encoder_hidden // 2),
            nn.ReLU(),
            nn.Dropout(p=self.dropout_p)
        )
        self.sess_mean_head = nn.Linear(self.sess_encoder_hidden // 2, self.s_dim)
        self.sess_logvar_head = nn.Linear(self.sess_encoder_hidden // 2, self.s_dim)

        ###################
        #### Inference ####
        ###################
        # 1. x and c to rnn
        self.input_embed = nn.Linear((self.x_dim + self.c_dim + self.s_dim), self.dense_x_gx)
        self.rnn_gx = nn.LSTM(self.dense_x_gx, self.dim_RNN_gx, self.num_RNN_gx, dropout=self.dropout_p if self.num_RNN_gx > 1 else 0.0)
        self.h_dropout = nn.Dropout(self.dropout_p)
        self.h_layernorm = nn.LayerNorm(self.dim_RNN_gx)
...
#         self.posterior_mlp = nn.Linear(self.z_dim, self.dense_ztm1_g)
        self.posterior_mlp = nn.Sequential(nn.Linear(self.z_dim, self.dense_h),
                                           ...
                                           nn.Linear(self.dense_h, self.dense_ztm1_g)
                                          )
        self.posterior_gru = nn.GRUCell(self.dim_RNN_gx, self.dense_ztm1_g)
        self.posterior_layernorm = nn.LayerNorm(self.dense_ztm1_g)
...
#                                           )
        ...
        self.cond_proj = nn.Sequential(nn.Linear(self.s_dim + self.c_dim, 64),
                                       nn.ReLU(),
                                       nn.Dropout(p=self.dropout_p),
                                       nn.Linear(64, self.dense_h * 2)
                                      )
        self.prior_mlp = nn.Sequential(nn.Linear(self.z_dim, 64),
                                       nn.ReLU(),
                                       nn.Dropout(p=self.dropout_p),
                                       nn.Linear(64, self.dense_h)
                                      )
...
#                                             )
        ...
        self.mlp_z_x = nn.Sequential(nn.Linear(self.z_dim + self.s_dim + self.c_dim, self.dense_h), 
                                     nn.ReLU(), 
                                     nn.Dropout(p=self.dropout_p))
...
#         )
    ...
    def infer_session(self, sample_x, sample_c):
        """
        Amortized q(s | sample_trials).
        sample_x: T_s x N_s x x_dim
        sample_c: T_s x N_s x c_dim
        Returns: s_mean (s_dim,), s_logvar (s_dim,)
        Note: use several trials (N_s = 5..50) from the same session.
        """
        T_s, N_s, _ = sample_x.shape
        #device = sample_x.device

        # Step 1: per-time-step embed: produce (T_s, N_s, embed_dim)
        xc = torch.cat([sample_x, sample_c], dim=-1)  # T_s x N_s x (x+c)
        # reshape to (T_s, N_s, hidden) via step embed applied to last dim
        step_emb = self.sess_step_embed(xc)  # T_s x N_s x 128

        # run LSTM per trial: since LSTM expects seq_len x batch x input_size, we can run LSTM over time where batch=N_s
        # transpose: seq_len x batch x input_size => feed into LSTM
        h_seq, (h_n, c_n) = self.sess_lstm(step_emb)  # h_seq: T_s x N_s x H

        # trial-level embedding: use last hidden state for each trial (h_seq[-1, :, :]) -> (N_s, H)
        trial_emb = h_seq[-1, :, :]  # N_s x H

        # Pool trial embeddings across N_s: mean pooling (or could use attention)
        pooled = trial_emb.mean(dim=0, keepdim=True)

        # MLP to produce mean & logvar for s
        h = self.sess_pool_mlp(pooled)  # 1 x H2
        s_mean = self.sess_mean_head(h)      # 1xs_dim
        s_logvar = self.sess_logvar_head(h)  # 1xs_dim
        return s_mean, s_logvar
    

    def inference(self, x, c, s_expand):
        ...
        x_comb = torch.cat([x, c, s_expand], dim=-1)
        x_g = self.input_embed(x_comb)
        h_seq, _ = self.rnn_gx(x_g)  
        ...
        for t in range(seq_len):
            ...
            g_z = self.posterior_layernorm(g_z)
    ...
    def generation_z(self, z_tm1, c_t, s):          # z_tm1 sarebbe z_{t-1}
        
        batch_size = z_tm1.shape[1]
        cond_inp = torch.cat([s, c_t], dim=-1)
        gamma, beta = self.cond_proj(cond_inp).chunk(2, dim=-1)
    ...
    def generation_x(self, z, c, s_expand):
        
        # 1. z_t to y_t
        seq_len, batch_size, _ = z.shape
        #s_expand = s.unsqueeze(0).expand(seq_len, -1, -1)  # T x B x s_dim
        inp = torch.cat([z, s_expand, c], dim=-1)  # T x B x (z + s + c)
        y = self.mlp_z_x(inp) 
    ...
    def forward(self, x, c, sample_for_s=None):
        
        # need input:  (seq_len, batch_size, x_dim)
        seq_len, batch_size, _ = x.shape
        if sample_for_s is not None:
            sample_x, sample_c = sample_for_s
            # sample_x: T_s x N_s x x_dim
            self.s_mean, self.s_logvar = self.infer_session(sample_x, sample_c)  # s_dim vectors
            # sample s
            s = self.reparameterization(self.s_mean, self.s_logvar)
            #s = s.unsqueeze(0).expand(seq_len, batch_size, -1)
            # we chose to sample using diagonal reparam here for s (s is diagonal Gaussian)
            # compute kl_s (per session scalar)
            # kl_s = kl_diag_gaussians(s_mean, s_logvar).mean()  # scalar
        else:
            # use prior p(s)=N(0,I)
...
#             kl_s = torch.tensor(0., device=device)
        s_expand = s.unsqueeze(0).expand(seq_len, batch_size, -1)
        self.z, self.z_mean, self.z_cov = self.inference(x, c, s_expand)
        z_0 = torch.zeros(1, batch_size, self.z_dim).to(self.device)
        z_tm1 = torch.cat([z_0, self.z[:-1, :,:]], dim=0)
        self.z_mean_p, self.z_cov_p = self.generation_z(z_tm1, c, s_expand)  
        gen_mean, gen_logvar = self.generation_x(self.z, c, s_expand)
