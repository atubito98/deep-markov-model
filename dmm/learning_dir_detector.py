...
class Dir_Detector(nn.Module):
    def __init__(self, input_dim=96, hidden_dim=128, num_layers=2, dropout=0.2, bidirectional=False):
...
class Learning_dir_detector():
    ...
    def create_loaders(self):
        ...
        if self.type == "z":
            ...
            train_data = inference_with_trials(self.dmm, train_set, cont_train, self.n_trials, self.device, chunk_size=128)
            vali_data = inference_with_trials(self.dmm, vali_set, cont_vali, self.n_trials, self.device, chunk_size=128)
            test_data = inference_with_trials(self.dmm, test_set, cont_test, self.n_trials, self.device, chunk_size=128)
