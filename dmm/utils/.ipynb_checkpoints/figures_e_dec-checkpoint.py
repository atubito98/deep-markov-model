...
def inference_with_trials_e_dec(dvae, data_set, cont_set, session_set, n_trials, input_dim, device, chunk_size=128):
    """
    ...
    #session_embed = dvae.session_embeddings(torch.from_numpy(session_set).long().to(device)).unsqueeze(0).expand(steps, -1, -1)
    # Output finale
    z_mean_accum = np.zeros((steps, n_samples, input_dim), dtype=np.float32)
    ...
    for start in range(0, n_samples, chunk_size):
        ...
        #s_chunk = session_embed[:, start:end].repeat_interleave(n_trials, dim=1)
        ...
        # Inferenza
        with torch.no_grad():
            z, z_mean, _ = dvae.inference(x_chunk, c_chunk)

        # Media sui trials (dim=2)
        z_mean_chunk = z.cpu().numpy().reshape(steps, batch_size, n_trials, input_dim).mean(2)

        # Inserisci nel buffer
        z_mean_accum[:, start:end, :] = z_mean_chunk

        torch.cuda.empty_cache()
        return np.transpose(z_mean_accum, (1, 0, 2))
...
def RT_pred_performance_e_dec(comm_dict, diff_dict):
...
def correlation_vs_gentime_e_dec(comm_dict, diff_dict):     
...
def PCA_vs_VAE_e_dec(comm_dict, diff_dict):
...
def latent_cn_traj_directions_e_dec(comm_dict, diff_dict):
...
def latent_traj_ctype_e_dec(comm_dict, diff_dict):
...
def plot_z_cRTtrue_e_dec(comm_dict, diff_dict):
...
def cs_ws_stop_plot_e_dec(comm_dict, diff_dict):
...
def cs_ws_GO_e_dec(comm_dict, diff_dict):
...
def change_trial_cs_e_dec(comm_dict, diff_dict):
...
def change_trial_cn_e_dec(comm_dict, diff_dict):
...
def gen_field_e_dec(comm_dict, diff_dict):
...
def consistent_sim_e_dec(comm_dict, diff_dict):
...
def autocorr_e_dec(comm_dict, diff_dict):
...
def dir_accuracy_gen_traj_e_dec(comm_dict, diff_dict, sim_start):
    dvae = comm_dict["dvae"]
    device = comm_dict["device"]
    tau = comm_dict["tau"]
    z_dim = comm_dict["z_dim"]
