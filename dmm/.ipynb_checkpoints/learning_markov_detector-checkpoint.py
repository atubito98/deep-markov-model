...
#                 delta_pred_arr = np.array(delta_pred_list)
# #                 NMSE_prop = ((total_loss / self.test_var).mean())/total_samples
#                 NMSE_prop = (total_loss.mean() / self.test_var.mean())/total_samples
        
        return train_loader_m, vali_loader_m, test_loader_m, train_loader_seq, vali_loader_seq, test_loader_seq, NMSE_prop

    
    def train(self):
        
        train_loader_m, vali_loader_m, test_loader_m, train_loader_seq, vali_loader_seq, test_loader_seq, self.NMSE_prop = self.create_loaders()
        ...
        print(f"MSE of the propagator: {self.NMSE_prop:4f}")
        ...
        for epoch in range(self.params["num_epochs"]):
            ...
            if vali_loss < best_loss:
                ...
                }, self.saved_dir + "/Markov_dict_" + self.type)
        ...
        for epoch in range(self.params["num_epochs"]):
            ...
            if vali_loss < best_loss:
                ...
                torch.save({
                    ...
                    'mean_NMSE': mean_NMSE_test,
                    'test_var': test_var,
                }, self.saved_dir + "/Non_Markov_dict_" + self.type)
...
#         NMSE_Markov = (test_loss_Markov/self.test_var).mean()
#         NMSE_NonMarkov = (test_loss_NonMarkov/self.test_var).mean()
