...
from .model import build_VAE, build_DKF_cx_causal_chot, build_DKF_session_hu

class LearningAlgorithm_session_hu():
    ...
    def build_model(self):
        ...
        elif self.model_name == 'DKF_session_hu':
            self.model = build_DKF_session_hu(cfg=self.cfg, device=self.device)
            self.best_model = build_DKF_session_hu(cfg=self.cfg, device=self.device)  
    ...
    def train(self):
        ...
        for epoch in range(start_epoch+2, epochs+1):
            ...
            for _, (batch_data, batch_gate_cont, _) in enumerate(train_dataloader):
                ...
                loss_tot = loss_recon + kl_warm * beta * loss_kl_z + kl_warm*loss_kl_s
            ...
            logger.info('Train => tot: {:.2f}, recon: {:.2f}, KL_z: {:.2f}, KL_s: {:.2f}; Val => tot: {:.2f}, recon: {:.2f}, KL_z: {:.2f}, KL_s: {:.2f}, lr: {:.2f}'.format(train_loss[epoch-1], train_recon[epoch-1], train_kl_z[epoch-1], train_kl_s[epoch-1], val_loss[epoch-1], val_recon[epoch-1], val_kl_z[epoch-1], val_kl_s[epoch-1], current_lr))
        ...
        with open(loss_file, 'wb') as f:
            ...
            pickle.dump([train_loss, val_loss, train_recon, train_kl_z, train_kl_s, val_recon, val_kl_z, val_kl_s], f)
        ...
        fig = plt.figure(figsize=(8,6))
        plt.rcParams['font.size'] = 12
        plt.plot(train_kl_z, label='Training')
        plt.plot(val_kl_z, label='Validation')
        plt.legend(fontsize=16, title='{}: KL Divergence for z latent space'.format(self.model_name), title_fontsize=20)
        plt.xlabel('epochs', fontdict={'size':16})
        plt.ylabel('loss', fontdict={'size':16})
        plt.semilogy()
        fig_file = os.path.join(save_dir, 'loss_KLD_z_{}.png'.format(tag))
        plt.savefig(fig_file)
